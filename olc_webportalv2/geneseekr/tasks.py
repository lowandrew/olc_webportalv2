...
def run_geneseekr(geneseekr_request_pk):
    ...
    try:
        geneseekr_dir = 'olc_webportalv2/media/geneseekr_{}'.format(geneseekr_request_pk)
        ...
        with open(os.path.join(geneseekr_dir, 'query.fasta'), 'w') as f:
        ...
        with open(batch_config_file, 'w') as f:
            ...
            f.write('INPUT:={} targets\n'.format(os.path.join(geneseekr_dir, 'query.fasta')))
        ...
        subprocess.Popen('AzureBatch -e {run_folder}/exit_codes.txt -c {run_folder}/batch_config.txt '
                         '-o olc_webportalv2/media'.format(run_folder=geneseekr_dir), shell=True)
